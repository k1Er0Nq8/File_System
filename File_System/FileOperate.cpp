...
void showDir()
{
    ...
    printf("当前目录下文件和目录总和:%d\n", unitAmount);
    printf("名称\t类型(0为目录1为文件)\t大小\tFCB块号\t文件起始盘块号\n");
    //遍历所有表项
...
int file_cp(char file1name[],char file2name[]){
    //同目录下先根据file2名字创建file2文件
    //然后读取file1内容，将file1的字符挨个放到file2里面
    
    //查找file1文件
    //printf("cp\n");
    int unitIndex1 = findUnitInTable(currentDirTable, file1name);
    if(unitIndex1 == -1)
    {
        printf("%s not found\n",file1name);
        return -1;
    }
    int type=currentDirTable->dirs[unitIndex1].type;
    if(type==0){
        printf("%s not a file.\n",file1name);
        return -1;
    }else{
    ...
    //找file2
    dirUnit unitTemp1 = currentDirTable->dirs[unitIndex1];
    int FCBBlock1 = unitTemp1.startBlock;
    FCB* fileFCB1 = (FCB*)getBlockAddr(FCBBlock1);
    int len=fileFCB1->fileSize;

    int unitIndex2 = findUnitInTable(currentDirTable, file2name);
    if(unitIndex2 == -1)
    {
        //文件2不存在，创建file2文件
        creatFile(file2name,len);
        unitIndex2 = findUnitInTable(currentDirTable, file2name);
    }
    dirUnit unitTemp2 = currentDirTable->dirs[unitIndex2];
    int FCBBlock2 = unitTemp2.startBlock;
    FCB* fileFCB2 = (FCB*)getBlockAddr(FCBBlock2);

    //读file1写file2
    int dataSize = fileFCB1->dataSize;
    //printf("datasize:%d\n",dataSize);
    char* data1 = (char*)getBlockAddr(fileFCB1->blockNum);
    //printf("Data1:%s",data1);
    //在不超出数据长度下，读取指定长度的数据
    for(int i=0; i<len && fileFCB1->readptr < dataSize; i++, fileFCB1->readptr++)
    {
        temp[i] = *(data1+fileFCB1->readptr);
        //printf("%c\n",temp[i]);
    }
    char* data2 = (char*)getBlockAddr(fileFCB2->blockNum);
    int contentLen = strlen(temp);
    //printf("%d\n",contentLen);
    int fileSize = fileFCB2->fileSize * block_szie;
    //printf("%d\n",fileSize);
    //在不超出文件的大小的范围内写入
    for(int i=0; i<contentLen && fileFCB2->dataSize<fileSize; i++, fileFCB2->dataSize++)
    {
        *(data2+fileFCB2->dataSize) = temp[i];
        //printf("%d\n",i);
    }
    if(fileFCB2->dataSize == fileSize)
        printf("file is full,can't write in\n");
    else
        printf("cp success.\n");
    return 0;
